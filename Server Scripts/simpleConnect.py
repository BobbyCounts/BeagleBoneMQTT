import paho.mqtt.client as mqtt
import datetime
import time
import requests
import json

import mysql.connector

# The callback for when the client receives a CONNACK response from the server.
def on_connect(client, userdata, flags, rc):
    print("Connected with result code "+str(rc))

    # Subscribing in on_connect() means that if we lose the connection and
    # reconnect then subscriptions will be renewed.
    client.subscribe("test/#")
	
	
def getNewToken():
	print("Refreshing key...")
	refreshToken = "1000.01086a40e0161b1fc604dcf1ffa421b1.8e166d4415070c2a339d530cf736b3f2"
	#lastRefresh = datetime.datetime.utcnow().timestamp()
	refresh_url = "https://accounts.zoho.eu/oauth/v2/token?refresh_token=1000.01086a40e0161b1fc604dcf1ffa421b1.8e166d4415070c2a339d530cf736b3f2&grant_type=refresh_token&client_id=1000.SQE0LHNJYJQ9G4U32A8QXKQPY0P0TR&client_secret=feee9a1746741a71d135b2742ab301dfe240b38617&redirect_uri=https://zylkerapps.com/oauth2callback&scope=ZohoMail.accounts.READ,ZohoMail.messages.CREATE"
	refresh = requests.post(refresh_url)
	if (refresh.status_code == 200):
		new_access_token = json.loads(refresh.text)
		print("Refresh Successful")
		return new_access_token['access_token']
	return 0	
	
	
def sendEmail(tempValue):
	global lastEmailTime
	getNewToken()
	lastEmailTime = time.time()
	print("Sending mail...")
	payload = {
		"fromAddress": "bobby@bobbycounts.com",
		"toAddress": "bobby@bobbycounts.com",
		"ccAddress": "",
		"bccAddress": "",
		"subject": "Temperature Alert",
		"content": "Temperature has exceeded the specified limit! The temperture at time of message was: " + str(tempValue),
		"askReceipt" : "no"
	}
	auth_header = {"Authorization" : "Zoho-oauthtoken " + access_token}
	email_url = "https://mail.zoho.eu/api/accounts/841028000000002002/messages"
	email_post = requests.post(email_url, headers=auth_header, json=payload)
	print("Mail sent successfully")
	

# The callback for when a PUBLISH message is received from the server.
def on_message(client, userdata, msg):
    global lastEmailTime
    crcLookup = [
    
    ]
    
    crc = 0
    payloadValue = int(msg.payload)
    
    crcValue = payloadValue & 0xFF
    
    for i in range(3,-1,-1):
         currentByte = (payloadValue & (0xFF << i*8)) >> i*8
         crc = crcLookup[currentByte ^ crc]
    
    if (crc):
        crcStatus = 0
    else:
        crcStatus = 1
    
    
    #Now we calculate temp in F
    
    rawTemp = (payloadValue&0xFFFFFF00)>>8
    
    calcuatedTemp = (rawTemp*130)/4095.0
    
    currentTime = datetime.datetime.utcnow().isoformat()
	
    mycursor.execute("SET time_zone = '+00:00'")
    print(msg.topic+ " | "+ msg.payload + " CRC: " + str(crcValue) + " | CRC Status: " + str(crcStatus))
    sql = "INSERT INTO Temperature (Time, Temp, CRC, CRCSTATUS) VALUES (%s, %s, %s, %s)"
    val = (str(currentTime), str(calcuatedTemp), str(crcValue), str(crcStatus))
    mycursor.execute(sql, val)
    mydb.commit()
    print("Record inserted.")
    if (calcuatedTemp > 120) and ((time.time() - lastEmailTime) > 60*15):
     sendEmail(calcuatedTemp)

client = mqtt.Client()
client.on_connect = on_connect
client.on_message = on_message


mydb = mysql.connector.connect(
  host="localhost",
  user="mqttServer",
  passwd="mariokart",
  database="mqttDatabase"
)

mycursor = mydb.cursor()


access_token = getNewToken()
if access_token == 0:
	print("Token cannot be obtained, exiting")
	exit()

lastEmailTime = 0.0	
	
client.connect("localhost", 1883, 60)


client.loop_forever()



	
